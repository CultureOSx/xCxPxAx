...
const CULTURES = [
  ...
  { label: "🇯🇵 Japanese",   tint: "#BC002D" },
  { label: "🇧🇷 Brazilian",  tint: "#009B3A" },
  { label: "🇻🇳 Vietnamese", tint: "#DA251D" },
];
...
const FEATURES = [
  ...
  { icon: "globe-outline" as const, color: "#3A86FF", title: "Cultural Map", desc: "See events, venues and culture hubs on a live interactive map." },
...
function Stat({ number, label, target }: { number: string; label: string; target: number }) {
  ...
  useEffect(() => {
    ...
    return () => { isMounted = false; animVal.removeAllListeners(); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
...
const CultureChip = memo(function CultureChip({
  ...
  const pressIn = useCallback(() =>
    Animated.spring(scale, { toValue: 0.93, useNativeDriver: true, friction: 5 }).start(), [scale]);

  const pressOut = useCallback(() =>
    Animated.spring(scale, { toValue: 1, useNativeDriver: true, friction: 5 }).start(), [scale]);
    ...
    <Animated.View style={{ transform: [{ scale }] }}>
      <Pressable
        style={({ pressed }) => [
          ...
          pressed && { backgroundColor: culture.tint + "18" },
        ]}
        onPressIn={pressIn}
        onPressOut={pressOut}
        onPress={handlePress}
    ...
    >
      ...
      >
        ...
        <FadeInView style={styles.heroContent}>
          ...
          {/* Social proof */}
          <FadeInView delay={500} style={styles.socialProof}>
      ...
      <FadeInView delay={100}>
        <View style={styles.sectionCenter}>
          ...
          {isDesktop ? (
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.cultureScrollRow}>
              {CULTURES.map((c) => (
                <CultureChip key={c.label} culture={c} styles={styles} colors={colors} />
              ))}
            </ScrollView>
      ...
      <FadeInView delay={120}>
        <View style={styles.section}>
          ...
          <View style={styles.problemGrid}>
            {PROBLEMS.map((p, i) => (
              <View key={p.text} style={styles.problemCard}>
