...
export default function LoginScreen() {
  ...
  const formContent = (
    <View style={s.cardInner}>
      ...
      <Animated.View entering={enter(165)} style={s.actionGroup} accessibilityRole="tablist">
        <Text style={[s.actionLabel, { color: colors.textSecondary }]}>Choose action</Text>
        <View style={s.actionRow}>
          {authActions.map((action) => {
            const active = action.key === 'signin';
            return (
              <Pressable
                key={action.key}
                style={[
      ...
      <Animated.View entering={enter(220)} style={s.form}>
        ...
        <View style={s.optionsRow}>
          <Checkbox
            ...
            label={<Text style={[s.rememberLabel, { color: colors.text }]}>Keep me signed in</Text>}
          />
        </View>
